...
def getAVideo(ID = ''):
    tmp = r.get(f'https://api.bilibili.com/x/web-interface/view?aid={ID[2:]}', headers = headers).json()
    if tmp['code'] != 0:
...
def getBVideo(ID = ''):
    tmp = r.get(f'https://api.bilibili.com/x/web-interface/view?bvid={ID}', headers = headers).json()
    if tmp['code'] != 0:
        err('s',tmp['code'],tmp['message'])
    else:
        tmp = tmp['data']
...
try:
    ...
    err('c', 1, 'ERR_INTERNET_CONNTCTION_FAILED\n无法连接网络')
